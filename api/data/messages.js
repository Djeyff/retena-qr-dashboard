...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    let qs = `order=timestamp.desc&limit=${Math.min(Number(limit), 200)}&offset=${Number(offset)}&select=*&chat_id=neq.status@broadcast&or=(from_me.eq.true,sender_id.not.ilike.*${OWNER}*)`;
